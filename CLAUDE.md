...
- `macbook-agent/` — standalone Node/Express daemon simulator + dashboard. `cd macbook-agent && npm install && npm start` serves the dashboard at `http://localhost:3000`; needs `adb forward tcp:8080 tcp:8080` for emulator connectivity.
